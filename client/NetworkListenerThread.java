...
public class NetworkListenerThread extends Thread {
	 ...
	 */
	public NetworkListenerThread(BufferedReader reader, Client client,
			GameWorld world) {
		this.reader = reader;
		this.world = world;
		this.client = client;
	...
	public void run() {
		try {
			...
			while (true) {
				...
				if (incoming == null)
					break;// indicates connection lost

				if (incoming.startsWith("upd")) { // if update
					String update = Database.unescapeNewLines(incoming
							.substring(4));
					if (debugMode)
						System.out.println("updated: " + update);
					try {
						WorldDelta.SERIALIZER.read(Tree.fromString(update))
								.apply(world);
					...
					String update = Database.unescapeNewLines(incoming
							.substring(4));
					try {
						WorldDelta.Action act = WorldDelta.SERIALIZER.read(
								Tree.fromString(update)).action();
